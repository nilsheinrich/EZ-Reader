...
def calc_loglik(fixations):
    ...
    # we groupby sentenceN as well to have the metric for every individual sentence
    groups_y_star = y_star.groupby(["runN", "sentenceN"])
    s_star = groups_y_star.apply(this_metric, ancorp).to_frame("stat")#.reset_index()
    # we pivot the table so that there is a column for every sentenceN for the metric
    s_star = s_star.pivot_table(["stat"], "runN", "sentenceN", dropna=False)
    return calc_likelihood(s_star, stat_s, "debug")
...
#parameter_names = ['Alpha1', 'Eta2', 'Epsilon', 'Lambda', 'Alpha2', 'Alpha3', 'Delta', 'I', 'A', 'M1', 'M2', 'S', 'X#i', 'Eta1', 'Psi', 'Omega1', 'Omega2', 'V', 'pF']

LL_table = pd.DataFrame({'parameters': par_ranges.get(parameter_name)})

# define all metrics
metrics = {
    1: Metrics.calc_meanFixPerSentence,
    2: Metrics.calc_SFD,
...
# load dataset y
y = pd.read_csv("Data/tmp.txt", sep="\t")
y = annotate_reading_data(y)  # calculate statistics of base data

# choose metric
#metrics_list = range(1, 15)
#metrics_list = [3, 5, 8, 13]
...
stat_s = groups.apply(this_metric, ancorp)
